...
$(function () {
    ...
    if (dataTablePermissions.length) {
        dt_permission = dataTablePermissions.DataTable({
            ajax: assetPath + 'api/admin/permissions',
            columns: [
            ...
            columnDefs: [
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        var roleBadgeObj = {
                            Admin:
                                '<a href="' +
                                userList +
                                '" class="me-50"><span class="badge rounded-pill badge-light-primary">Admin</span></a>',
                            Manager:
                                '<a href="' +
                                userList +
                            ...
                            Restricted:
                                '<a href="' +
                                userList +
                                '" class="me-50"><span class="badge rounded-pill badge-light-danger">Restricted</span></a>'
                        };
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return (
                            '<button data-id="' + full['id'] + '" class="btn btn-sm btn-icon" data-bs-toggle="modal" data-bs-target="#editPermissionModal">' +
                            feather.icons['edit'].toSvg({class: 'font-medium-2 text-body'}) +
                            '</i></button>' +
                            '<button data-id="' + full['id'] + '" class="btn btn-sm btn-icon delete-record">' +
            ...
            submitHandler: function (form) {
                $.ajax({
                    ...
                    success: function (response) {
                        dt_permission.rows.add(response.data).draw();
                        $('.modal').modal('hide');
                    }
                });
            }
    ...
    if (editPermissionForm.length) {
        editPermissionModal.on('shown.bs.modal', function (e) {
            let button = $(e.relatedTarget);
            let tr = button.parents('tr');
            $.ajax({
                url: assetPath + 'api/admin/permissions/' + button.data('id'),
                type: 'GET',
                data: {},
                success: function (response) {
                    editPermissionForm.find('#editPermissionName').val(response.data.name)
                    editPermissionForm.attr('action', assetPath + 'api/admin/permissions/' + button.data('id'))
                }
            });
    ...
    $('.datatables-permissions tbody').on('click', '.delete-record', function () {
        let self = $(this)
        $.ajax({
            url: assetPath + 'api/admin/permissions/' + self.data('id'),
            type: 'DELETE',
            data: {},
            success: function (response) {
                dt_permission.row(self.parents('tr')).remove().draw();
            }
        });
    });
